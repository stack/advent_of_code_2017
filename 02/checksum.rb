...
def checksum_spreadsheet(data)
  rows = data.each_line.map do |line|
    line.split(' ').map(&:to_i)
  end

  row_values = rows.map do |row|
    (min, max) = min_max row
    max - min
  end
...
def checksum_spreadsheet_2(data)
  rows = data.each_line.map do |line|
    line.split(' ').map(&:to_i)
  end

  row_values = rows.map do |row|
    (smaller, larger) = dividable row
    larger / smaller
  end
...
def dividable(row)
  ...
  while !sorted_row.empty?
    smaller = sorted_row.shift

    sorted_row.each do |larger|
      return [smaller, larger] if larger % smaller == 0
    end
  end

  raise 'Could not find a matching dividable set'
end
...
example_checksum = checksum_spreadsheet EXAMPLE_SPREADSHEET
puts "Example Checksum: #{example_checksum}"

checksum = checksum_spreadsheet SPREADSHEET
puts "Checksum: #{checksum}"

example_checksum = checksum_spreadsheet_2 EXAMPLE_SPREADSHEET
puts "Example Checksum 2: #{example_checksum}"

checksum = checksum_spreadsheet_2 SPREADSHEET
puts "Checksum #{checksum}"
